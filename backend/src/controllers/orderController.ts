...
export const createOrder = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { customerId, orderDate, deliveryDate, items, advancePayment, notes } = req.body;
    ...
    await emailService.sendOrderConfirmation(fullOrder!, customer);
...
export const getOrderStats = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const completedOrders = await Order.count({ where: { status: OrderStatus.COMPLETED } });
