...
export default function InvoiceGenerator({ onClose, existingInvoice }: InvoiceGeneratorProps) {
...
${invoiceData.items.map(item => `- ${item.name} (Qty: ${item.quantity}) - ${formatCurrency(item.price * item.quantity)}`).join('\n')}

Total Amount: ${formatCurrency(calculateTotal())}
Estimated Delivery: ${new Date(invoiceData.estimatedDelivery).toLocaleDateString()}
...
*Items:*
${invoiceData.items.map(item => `• ${item.name} (${item.quantity}x) - ${formatCurrency(item.price * item.quantity)}`).join('\n')}
...
Thank you for choosing Cuttoshape! 🪡✂️`;
    ...
    const whatsappLink = `https://wa.me/${invoiceData.customerPhone.replace(/[^0-9]/g, '')}?text=${encodeURIComponent(message)}`;
      ...
      <div className="px-4 pb-20">
        ...
        {step === 1 && (
          <div className="space-y-4">
            ...
            <button
              onClick={() => setStep(2)}
              disabled={!invoiceData.customerName || !invoiceData.customerEmail || !invoiceData.customerPhone}
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <div className="flex space-x-2">
                <button
                  ...
                  className={`px-3 py-1 rounded text-sm ${currency === 'NGN' ? 'bg-indigo-600 text-white' : 'bg-gray-200 text-gray-600'}`}
                >
                  NGN
                </button>
                <button
                  onClick={() => setCurrency('USD')}
                  className={`px-3 py-1 rounded text-sm ${currency === 'USD' ? 'bg-indigo-600 text-white' : 'bg-gray-200 text-gray-600'}`}
                >
                  USD
                </button>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setInvoiceData(prev => ({ ...prev, profit: parseFloat(e.target.value) || 0 }))}
                    className="w-full p-2 border border-gray-200 rounded text-sm"
                  />
                </div>
              </div>
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setInvoiceData(prev => ({ ...prev, estimatedDelivery: e.target.value }))}
                className="w-full p-3 border border-gray-200 rounded-lg"
              />
            </div>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm">
              <div className="text-center mb-6">
                <h1 className="font-['Pacifico'] text-2xl text-indigo-600 mb-2">Cuttoshape</h1>
              ...
              <div className="grid grid-cols-2 gap-6 mb-6">
                <div>
                  ...
                  <p className="text-gray-600 text-sm">{invoiceData.customerPhone}</p>
                  {invoiceData.customerAddress && (
                    <p className="text-gray-600 text-sm mt-1">{invoiceData.customerAddress}</p>
                  )}
                </div>
                <div className="text-right">
                  ...
                  <div className="mb-2">
                    ...
                    <span className="font-medium">{new Date(invoiceData.expiryDate).toLocaleDateString()}</span>
                  </div>
                  <div>
                    <span className="text-gray-600 text-sm">Delivery: </span>
                    <span className="font-medium">{new Date(invoiceData.estimatedDelivery).toLocaleDateString()}</span>
                  </div>
                </div>
              </div>
                  ...
                  <div className="w-64">
                    ...
                    <div className="border-t border-gray-200 pt-2 mt-2">
                      <div className="flex justify-between">
                        ...
                        <span className="text-lg font-bold text-indigo-600">{formatCurrency(calculateTotal())}</span>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="mt-6 text-center text-sm text-gray-600">
                ...
                <p className="mt-2">This is a proforma invoice. Payment is due upon acceptance.</p>
              </div>
            </div>
