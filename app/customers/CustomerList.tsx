...
export default function CustomerList({ searchQuery, filterStatus, onSelectCustomer }: CustomerListProps) {
  ...
  const getStatusBadge = (customer: any) => {
    ...
    if (totalSpent > 5000) {
      return { label: 'VIP', color: 'bg-purple-100 text-purple-700', dotColor: 'bg-purple-500' };
    }
    if (customer.orderCount > 0) {
      return { label: 'ACTIVE', color: 'bg-green-100 text-green-700', dotColor: 'bg-green-500' };
    }
    return { label: 'NEW', color: 'bg-blue-100 text-blue-700', dotColor: 'bg-blue-500' };
  };
            ...
            >
              <div className="flex items-center space-x-3">
                <div className="relative">
                  ...
                  <div className={`absolute -bottom-1 -right-1 w-4 h-4 rounded-full border-2 border-white ${badge.dotColor}`}></div>
                </div>
